  ...
  #include "myrabot_arm_base_b/SwitchesSecuence.h"
  ...
  myrabot_arm_base_b::WriteServos move, move_acercar;
  ...
  myrabot_arm_base_b::SwitchesSecuence secuencia;
  ...
  void secuenciaInterruptores(const myrabot_arm_base_b::SwitchesSecuence& secuence)
  {   
  ...
  bool buscarInterruptor(geometry_msgs::Pose base_pose_0)
  {
	...
	static double angulo_1 = 0;
	static double angulo_2 = 0.6;
	...
	std::cout<<angulo_1<<" - "<<angulo_2<<std::endl;
	...
	if (moverDistanciaBase(base_pose_0, avance, 3) || distancia)
	{
		...
		if (girarAnguloBase(sentido, 0, base_pose_0.orientation.z, 0.2, 0.08))
		{
			...
			if (sentido == -1)
			{angulo_1 = angulo_1 + 0.2;}
			else
			{angulo_2 = angulo_2 - 0.2;}
  ...
  int main(int argc, char **argv)
  {
	...
	if(::secuencia_recibida && ::start_task && objetivo < ::secuencia.switches.size())
	{
				...
				//{
					...
					//{
					  ...
					  if (colocar == 12)
					  {
						  ...
						  alcance = dentroAlcance(::punto, orient, ::inclinacion_pinza, ::p, 0, "dedo");
		...
		else if (colocar > 12 && !orientando_panel)
		{
			j = 0;
			::cont = 0;
			::cont_1 = 0;
